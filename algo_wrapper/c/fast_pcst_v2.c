...
void get_sum_on_edge_part(
        PCST *pcst, int edge_part_ind, double *total_sum,
        double *finished_moat_sum, int *cur_c_ind) {
    int endpoint = pcst->edges[right(edge_part_ind)].first, ii;
    if (edge_part_ind % 2 == 1) {
        endpoint = pcst->edges[right(edge_part_ind)].second;
    }
    *total_sum = 0.0;
    *cur_c_ind = endpoint;
    int path_size = 0;
...
void mark_clusters_as_necessary(PCST *pcst, int start_c_ind) {
    int cur_c_ind = start_c_ind;
    while (!pcst->clusters[cur_c_ind].necessary) {
        pcst->clusters[cur_c_ind].necessary = true;
        if (pcst->clusters[cur_c_ind].merged_into >= 0) {
            cur_c_ind = pcst->clusters[cur_c_ind].merged_into;
        } else {
            return;
        }
...
void mark_nodes_as_deleted(PCST *pcst, int start_node_ind, int parent_n_ind) {
    pcst->node_deleted[start_node_ind] = true;
    int ii, queue_size = 0, queue_ind = 0;
    pcst->c_queue[queue_size++] = start_node_ind;
    while (queue_ind < queue_size) {
        int cur_n_index = pcst->c_queue[queue_ind++]; // cur_node_index
...
void label_final_comp(PCST *pcst, int start_n_ind, int new_comp_ind) {
    ...
    while (queue_next < queue_size) {
        int cur_node_ind = pcst->c_queue[queue_next++];
        int cur_size = pcst->final_comp[new_comp_ind].size++;
        pcst->final_comp[new_comp_ind].array[cur_size] = cur_node_ind;
        if (cur_node_ind == pcst->root) {
            pcst->root_comp_ind = new_comp_ind;
        }
        for (int ii = 0; ii < pcst->p3_nei[cur_node_ind].size; ++ii) {
            int n_node_ind = pcst->p3_nei[cur_node_ind].array[ii].first;
...
bool pcst(const EdgePair *edges, const double *prizes,
    ...
    if (pruning == SimplePruning) {
        simple_pruning(pcst, result_nodes, result_edges);
    } else if (pcst->pruning == GWPruning) {
        gw_pruning(pcst, result_nodes, result_edges);
    } else if (pcst->pruning == StrongPruning) {
        for (int ii = 0; ii < pcst->g; ii++) {
            pcst->final_comp[ii].size = 0;
            pcst->final_comp[ii].array = malloc(
                    sizeof(int) * (pcst->p2_re_size + 1));
        }
        strong_pruning(pcst, result_nodes, result_edges);
        for (int ii = 0; ii < pcst->g; ii++) {
        ...
        no_pruning(pcst, result_nodes, result_edges);
    }
    for (int ii = 0; ii < pcst->n; ii++) {
...
bool no_pruning(PCST *pcst, Array *result_nodes, Array *result_edges) {
...
bool simple_pruning(PCST *pcst, Array *result_nodes, Array *result_edges) {
...
bool gw_pruning(PCST *pcst, Array *re_nodes, Array *re_edges) {
    ...
    for (int ii = pcst->p2_re_size - 1; ii >= 0; ii--) {
        ...
        if (pcst->edge_info[cur_e_ind] < 0) {
            ...
            if (pcst->clusters[inact_c_ind].necessary) {
                ...
                mark_nodes_as_deleted(pcst, inact_side_node, active_side_node);
            }
        }
    }
    for (int ii = 0; ii < pcst->n; ii++) {
        if (!pcst->node_deleted[ii] && pcst->node_good[ii]) {
            re_nodes->array[re_nodes->size++] = ii;
        }
    }
    for (int ii = 0; ii < phase3_result_size; ii++) {
        re_edges->array[re_edges->size++] = phase3_result[ii];
    }
    return true;
}

bool strong_pruning(PCST *pcst, Array *result_nodes, Array *result_edges) {
